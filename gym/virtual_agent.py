"""
Current:
    Acting completely greedily now    
"""
...
def get_action(games_played, state_num, initial_epsilon=0.8, min_epsilon=0, decay_rate=0.01):
...
def calculate_rewards(prev_state, curr_state, agent):
    ...
    distance_x = (prev_x_dist - curr_x_dist) if curr_x_pos != prev_x_pos else 0
    distance_y = (prev_y_dist - curr_y_dist) if curr_y_pos != prev_y_pos else 0
...
def update_q_value(agent, prev_state_num, action, reward, next_state_num, learning_rate=0.1, discount_factor=0.5):
...
def q_learning_step(agent, prev_state_num, prev_state_values, action, curr_state_num, curr_state_values):
    ...
    dist_x, dist_y, damage_taken, damage_done, _, _ = calculate_rewards(prev_state_values, curr_state_values, agent)
    
    # More extreme penalties for moving away
    if dist_x < 0: dist_x *= 2
    if dist_y < 0: dist_y *= 2
    damage_reward = (damage_done * 2) - damage_taken
    distance_reward = (dist_x + dist_y) // 2

    nothing_reward = -50 if not any([dist_x, dist_y, damage_done]) else 0 # Extreme penalty for doing nothing
    reward = damage_reward + distance_reward + nothing_reward

    update_q_value(agent, prev_state_num, action, reward, curr_state_num)

def update_odds_long(agent, stock_lost, stock_taken, actions_long):
    stock_reward = 5
    if stock_lost:
...
def train(games, save_frequency, display):
    total_games = get_game_num()
    for i in range(games):
        print(f'Game: {total_games + i}/{games}')
...
train(games=10000, save_frequency=10, display=False)
