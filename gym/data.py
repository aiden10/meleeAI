...
x_pos_ranges = [
    (-5000, -85), (-85, -75), (-75, -65), (-65, -55), (-55, -45), (-45, -35),
    (-35, -25), (-25, -15), (-15, -5), (-5, 5), (5, 15), (15, 25), (25, 35),
    (35, 45), (45, 55), (55, 65), (65, 75), (75, 85), (85, 5000)]
...
def create_initial_agent():
    ...
    for state_id, state in enumerate(states, start=1):
        ...
        if state == (-1000, -85): # offstage and at edge of stage (left side)
            action_rewards["L_Dash"] += 15  # Make L_Dash less likely
            action_rewards["L_Walk"] += 15  # Make L_Walk less likely
            action_rewards["R_Dash"] += 15  # Make R_Dash more likely
            action_rewards["R_Walk"] += 15  # Make R_Walk more likely
            action_rewards["Jump"] += 15    # Make jumps more likely
        
        elif state == (85, 1000): # offstage and at edge of stage (right side)
            action_rewards["R_Dash"] += 15  # Make R_Dash less likely
            action_rewards["R_Walk"] += 15  # Make R_Walk less likely
            action_rewards["L_Dash"] += 15  # Make L_Dash more likely
            action_rewards["L_Walk"] += 15  # Make L_Walk more likely
            action_rewards["Jump"] += 15  # Make jumps more likely
                    
        data[state_id] = {"State": state, "Actions": action_rewards}
