...
def create_initial_agent():
    ...
    for state_id, state in enumerate(states, start=1):
        ...
        if state == (-1000, -85): # offstage and at edge of stage (left side)
            action_rewards["L_Dash"] += 5  # Make L_Dash less likely
            action_rewards["L_Walk"] += 5  # Make L_Walk less likely
            action_rewards["R_Dash"] += 5  # Make R_Dash more likely
            action_rewards["R_Walk"] += 5  # Make R_Walk more likely
            action_rewards["Jump"] += 5    # Make jumps more likely
        
        elif state == (85, 1000): # offstage and at edge of stage (right side)
            action_rewards["R_Dash"] += 5  # Make R_Dash less likely
            action_rewards["R_Walk"] += 5  # Make R_Walk less likely
            action_rewards["L_Dash"] += 5  # Make L_Dash more likely
            action_rewards["L_Walk"] += 5  # Make L_Walk more likely
            action_rewards["Jump"] += 5  # Make jumps more likely
