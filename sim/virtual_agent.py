...
def get_action(state_num):
    ...
    # print(f'actions: {list(actions.keys())} | probabilities: {softmaxed_probabilities}')
        ...
        for action in action_list:
            if action != "Release":
                ...
                if not (dist_x or dist_y or damage_done) and action in ATTACK_LIST:
                    ...
                    for move_action in ['L_Dash', 'R_Dash', 'L_Walk', 'R_Walk']: 
...
"""
learning rate adjustments if no improvement is happening. After 500 games look at the average damage of the first and last 100. If there is no increase
or a decrease, adjust the learning rate values randomly?
shouldn't always just do the action with the highest value. It should still be probability based with those values
